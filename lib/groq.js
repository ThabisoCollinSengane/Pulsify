...
// Current Groq chat models — preference order, fall back on model-not-found errors
const CHAT_MODELS = [
...
async function groqChat(messages, systemPrompt) {
  ...
  for (const model of CHAT_MODELS) {
    try {
      ...
      lastErr = e;
      const msg = e.message || '';
      // Try next model on: not-found, deprecated, 5xx (overloaded/unavailable), or network errors
      const canFallback = /404|model_not_found|decommissioned|deprecated|Groq 5\d\d|overload|unavailable|service.?unavail|fetch failed|ECONNECT|ETIMEDOUT/i.test(msg);
      if (!canFallback) throw e;
      console.warn(`[groq] model ${model} unavailable, trying next:`, msg.slice(0, 120));
    }
  }
  throw lastErr;
}

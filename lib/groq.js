...
// Model preference order — try the first, fall back if it returns 404/400 model errors
const CHAT_MODELS = [
...
async function groqChat(messages, systemPrompt) {
  ...
  for (const model of CHAT_MODELS) {
    try {
      ...
      lastErr = e;
      // Only retry on model-not-found errors; propagate auth/rate-limit errors immediately
      const msg = e.message || '';

...
function buildLumiSystemPrompt(event, channel) {
  ...
  const channelNote = channel === 'whatsapp'
    ? 'FORMAT: WhatsApp — max 2–3 SHORT sentences per reply. Plain text only, zero markdown, no bullet points, no asterisks. Use 1 emoji naturally per reply.'
    : 'FORMAT: Web chat — 3–5 sentences MAX. Use \\n\\n between separate thoughts (paragraph breaks). Never write one long wall of text. Use 1–2 relevant emojis per reply naturally. No bullet-point dumps, no asterisks.';

...
async function groqChat(messages, systemPrompt) {
  ...
  for (const model of CHAT_MODELS) {
    try {
      // On 429 rate-limit, wait 2s and retry the same model once before failing
      try {
      ...
      lastErr = e;
      const msg = e.message || '';
      // Only try the next model on model-not-found / deprecated errors
      if (!/404|model_not_found|decommissioned|deprecated/i.test(msg)) throw e;

  ...
  <tr><td align="center">
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px">
      ...
      <tr><td style="padding-bottom:28px">
        ...
        <span style="font-size:26px;font-weight:900;letter-spacing:-1px;color:#ffffff"> PULSIFY</span>
      </td></tr>
      ...
      <tr><td style="padding-top:28px;text-align:center;font-size:12px;color:#4a4a4a;line-height:1.8">
        <p style="margin:0">© ${YEAR} Pulsify · South Africa</p>
        <p style="margin:4px 0 0">This email was sent because you have a Pulsify account.
        <a href="${APP_URL}" style="color:#4a4a4a;text-decoration:underline">Visit Pulsify</a></p>
      </td></tr>
...
function welcomeHtml(displayName) {
  ...
  return layout(`
    ${card(`
      ...
      <h1 style="margin:0 0 12px;font-size:24px;font-weight:800;color:#ffffff;line-height:1.3">Welcome to Pulsify, ${name}!</h1>
      <p style="margin:0 0 20px;font-size:15px;color:#a0a0a0;line-height:1.7">You're in. Pulsify is South Africa's go-to platform for discovering events, connecting with friends, and keeping up with what's happening near you.</p>
  ...
  return layout(`
    ${card(`
      ...
      <p style="margin:0 0 16px;font-size:15px;color:#a0a0a0;line-height:1.7">
        Your Pulsify identity has been reviewed and <strong style="color:#00e5a0">approved</strong>. A verified badge now appears on your profile, visible to everyone on Pulsify.
      </p>
  ...
  return layout(`
    ${card(`
      ...
      <p style="margin:0 0 12px;font-size:15px;color:#a0a0a0;line-height:1.7">
        Thank you for applying for verification on Pulsify. Unfortunately, your application was not approved at this time.
      </p>
      ${reasonBlock}
...
async function sendWelcomeEmail(to, displayName) {
  await send(to, 'Welcome to Pulsify 🎉', welcomeHtml(displayName));
}

async function sendVerifApprovedEmail(to, displayName) {
  await send(to, "You're Verified on Pulsify ✓", verifApprovedHtml(displayName));
}

async function sendVerifRejectedEmail(to, displayName, notes) {
  await send(to, 'Your Pulsify Verification Update', verifRejectedHtml(displayName, notes));
}

module.exports = { sendWelcomeEmail, sendVerifApprovedEmail, sendVerifRejectedEmail };

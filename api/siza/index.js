...
module.exports = async (req, res) => {
  ...
  try {
    ...
    /* ─── GET /siza/health — Gemini connectivity check ──────── */
    if (url === '/siza/health' && req.method === 'GET') {
      ...
      if (!geminiKey) {
        ...
        try {
          await geminiChat([{ role: 'user', content: 'Reply with exactly: pong' }], 'You are a test assistant.');
          results.gemini = 'OK';
        } catch (e) {
          results.gemini = `ERR: ${(e.message || '').slice(0, 100)}`;
        }
      }

const { sb, sbAs, authUser, tokenFrom, CORS, verifyToken } = require('../shared');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (squadActionMatch && squadActionMatch[2] === 'invite' && req.method === 'POST') {
      ...
      const { error } = await sbAs(token).rpc('insert_squad_invite', { p_squad_id: squadId, p_inviter_id: auth.user.id, p_invitee_id: inviteeId });
      if (error) return res.status(400).json({ error: error.message });

...
/* ═══════════════════════════════════════════════════════════
   SUPABASE SESSION SYNC — AUTO-LOAD PROFILE ON PAGE LOAD
   ═══════════════════════════════════════════════════════════ */

async function syncProfileWithSupabase() {
  console.log('[storage] Syncing profile with Supabase...');
  ...
  try {
    ...
    const sessionRes = await fetch(`${SUPA_URL}/auth/v1/user`, {
    ...
    if (sessionRes.status === 200) {
      ...
      if (user && user.id) {
        ...
        console.log('[storage] Profile synced:', profile.display_name);
        return profile;
      }
    }
  } catch (e) {
    console.warn('[storage] Sync failed (not logged in or API error):', e.message);
  }
  
  return null;
}

// Auto-run sync when page loads (only if we're not on signin/create pages)
...
window._pulsifyStorage = {
  syncProfile: syncProfileWithSupabase,
  apiFetch,
  getToken,
  setToken
};
